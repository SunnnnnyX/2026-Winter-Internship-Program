...
DATE_END   = "2026-02-06"
OUT_XLSX = Path(r"D:\batch_exec_backtest_results_with_TP_T.xlsx")
...
# 加仓触发（先按你之前的“苛刻一点”的版本：在触发价基础上，再要求 MA5/MA10 都走强）
ADD_ATR = 1.0
...
USE_TP_T = True
TP_SELL_FRAC = 0.50  # 止盈卖出一半（按当前持仓股数的一半）

# TP-A 触发：收盘从“在MA10上方”变为“跌破MA10”，且当下仍是盈利（避免亏着卖一半）
TP_REQUIRE_PROFIT = True

# 做T回补（三条件）：收盘确认 -> 次日开盘回补
# 1) close > MA10
# 2) MA10 上行
# 3) close > 昨日 high（重新走强）
# =================================================================

# 过滤：次日开盘确认是否要求 close>=MA5（可选）
...
def backtest_one_stock(df: pd.DataFrame, ts_code: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    返回：
      equity_df
      trades_df      (只记录“最终清仓”的一笔，代表一次完整持仓周期)
      inflows_df     (所有买入：entry / add_1 / add_2 / t_rebuy)
      events_df      (TP卖出/回补/加仓等时间点与信号，便于你核对)
    """
    ...
    def record_inflow(kind: str, inflow_date, price, value, qty, tranches_after: int):
        inflows.append({
            "ts_code": ts_code,
            "trade_id": trade_id,
            "inflow_type": kind,                 # entry / add_1 / add_2 / t_rebuy
    ...
    def record_event(evt_type: str, evt_date, side: str, price: float, qty: float, reason: str, extra: dict | None = None):
        d = {
            "ts_code": ts_code,
            "trade_id": trade_id,
            "event_type": evt_type,          # tp_sell_half / t_rebuy / add / entry / exit
    ...
    def do_full_exit(next_row, exit_price, reason):
        ...
        nonlocal pending_rebuy_qty, last_tp_date, trade_start_equity
    ...
    for i in range(len(df)):
        ...
        if in_pos:
            ...
            if USE_TP_T and pd.notna(row.get("ma10", np.nan)):
                ...
                # 1) 若已经卖出一半，满足回补条件（三条件）-> 次日开盘回补同等股数
                if pending_rebuy_qty > 0:
                    cond1 = close > ma10
                    cond2 = pd.notna(prev_ma10) and (ma10 > prev_ma10)
                    cond3 = close > float(row["high"]) if False else (close > float(df.loc[i, "high"].shift(1)) if False else close > float(df.loc[i-1, "high"]) if i-1 >= 0 else False)
                    # 上面 cond3 写得很绕，直接用昨日high：
                    cond3 = (i-1 >= 0) and (close > float(df.loc[i-1, "high"]))

                    if cond1 and cond2 and cond3:
                        buy_px = apply_cost(next_open, "buy")

                        # 仍然受 MAX_EXPOSURE 市值上限约束
                        ...
                        if can_value > 0:
                            ...
                            record_event("t_rebuy", next_row["date"], "BUY", buy_px, buy_qty,
                                         "t_rebuy_cond(ma10_up & close>ma10 & close>yhigh)",
                                         {"cond1_close_gt_ma10": cond1, "cond2_ma10_up": cond2, "cond3_close_gt_yhigh": cond3})

                    # 做T回补完成/未完成都不影响后面加仓，但为了清晰：有 pending 时先不加仓
                    continue

                # 2) 若尚未做T（pending=0），触发 TP-A：从MA10上方跌破MA10 -> 次日开盘卖出一半
                ...
                if prev_above and now_below and profitable and shares > 0:
                    ...
                    if sell_qty > 0:
                        ...
                        # 说明：这里不改变 tranches；这是“做T仓位”，不是计划加仓档位
                        continue

            # ---- 计划加仓：15% -> 10% -> 补满到50%（收盘确认->次日开盘加）----
            # “苛刻条件”：触发价 + MA5上行 + MA10上行（更贴合你说的：MA5以上、趋势好才追加）
            if tranches < MAX_TRANCHES and pd.notna(row["atr"]) and pd.notna(avg_cost):
                ...
                cond_ma5 = pd.notna(row["ma5"]) and (float(row["close"]) > float(row["ma5"])) and (float(row["ma5"]) > float(df.loc[i-1, "ma5"]) if i-1 >= 0 and pd.notna(df.loc[i-1, "ma5"]) else True)
                cond_ma10 = pd.notna(row["ma10"]) and (float(row["close"]) > float(row["ma10"])) and (float(row["ma10"]) > float(df.loc[i-1, "ma10"]) if i-1 >= 0 and pd.notna(df.loc[i-1, "ma10"]) else True)
                ...
                if cond_price and cond_ma5 and cond_ma10:
                    ...
                    current_value = shares * next_open  # 市值口径
                    ...
                    if add_value > 0:
                        ...
                        new_cost = (avg_cost * shares + buy_px * add_qty) / (shares + add_qty)
                        shares += add_qty
                        cash -= cost
                        avg_cost = new_cost
                        tranches += 1
        ...
        if bool(row.get("signal_vA", False)):
            ...
            trade_id += 1
            trade_start_equity = cash + shares * float(row["close"])  # 开仓前权益（通常就是cash）
    ...
    return pd.DataFrame(eq_rows), pd.DataFrame(trades), pd.DataFrame(inflows), pd.DataFrame(events)
...
def main():
    ...
    try:
        for idx, ts in enumerate(ts_codes, 1):
            ...
            equity, trades, inflows, events = backtest_one_stock(df, ts_code=ts)
            if equity.empty:
                continue
            ...
            if inflows is not None and len(inflows):
                f2 = inflows.copy()
