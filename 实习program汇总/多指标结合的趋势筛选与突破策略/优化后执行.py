...
DATE_END   = "2026-02-06"
OUT_XLSX = Path(r"D:\batch_exec_backtest_results_with_inflows.xlsx")
...
# 加仓触发（仍然用 avg_cost + ADD_ATR*ATR，收盘确认->次日开盘加）
ADD_ATR = 1.0
...
# MA5 风控（你最新：滚动规则改成 7天3次）
FAST_WIN_DAYS = 3
FAST_WIN_BREACH = 2        # 入场后三天内，有两天收盘 < MA5 => 退出

ROLL_WIN_DAYS = 7
ROLL_WIN_BREACH = 3        # 持仓期间，7天窗口内有3天收盘 < MA5 => 退出
...
# 过滤：次日开盘确认是否要求 close>=MA5（可选，不想要就 False）
...
def backtest_one_stock(df: pd.DataFrame, ts_code: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    返回：
      equity_df
      trades_df
      inflows_df  (新增：每次开仓/加仓的资金流入明细)
    """
    ...
    for i in range(len(df)):
            ...
            if tranches < MAX_TRANCHES and pd.notna(row["atr"]) and pd.notna(avg_cost):
                ...
                if float(row["close"]) >= add_trigger:
                    ...
                    if add_value > 0:
                        ...
                        # ★记录资金流入
                        record_inflow("add", next_row["date"], buy_px, value, add_shares, tranches)
        ...
        if bool(row.get("signal_vA", False)):
            ...
            # ★记录资金流入
            record_inflow("entry", next_row["date"], buy_px, value, buy_shares, tranches)

    # ============ 期末强平 ============
    if FORCE_EXIT_AT_END and in_pos and shares > 0:
        last_row = df.iloc[-1]
        do_exit(last_row, float(last_row["close"]), "end_of_period")
    ...
    return pd.DataFrame(eq_rows), pd.DataFrame(trades), pd.DataFrame(inflows)
...
def main():
    ...
    try:
        for idx, ts in enumerate(ts_codes, 1):
            ...
            equity, trades, inflows = backtest_one_stock(df, ts_code=ts)
            if equity.empty:
                continue
    ...
    print(f"\nSaved: {OUT_XLSX}")
    print(f"Stocks: {len(per_stock)}; Trades rows: {len(trades_df)}; Inflows rows: {len(inflows_df)}")
