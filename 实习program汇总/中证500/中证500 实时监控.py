...
INDEX_CODE = "000905"  # 中证500指数代码 可调市场代码
SINA_SYMBOL_PRIMARY = "sh000905"
SINA_SYMBOL_FALLBACK = "sz399905"

# 是否优先尝试东方财富的 30min 区间接口（能拿多日数据）
USE_EASTMONEY_M30 = True

REFRESH_SEC = 60             # 延时 1min 刷新
LOOKBACK_DAYS_DAILY = 800    # 日线拉取回看天数
LOOKBACK_DAYS_M30 = 20       # 30min 拉取回看天数
...
MULTIPLIER = 1.0  # 指数本身没有合约乘数；若是中证500期货可改成 200
...
def fetch_daily(now_cn: datetime) -> pd.DataFrame:
    start = (now_cn - timedelta(days=LOOKBACK_DAYS_DAILY)).date()
    last_err = None
    for sym in (SINA_SYMBOL_PRIMARY, SINA_SYMBOL_FALLBACK):
        try:
            df = ak.stock_zh_index_daily(symbol=sym)
            if df is None or len(df) == 0:
                raise ValueError(f"stock_zh_index_daily 返回空数据: {sym}")
            df = df.rename(columns={"date": "date", "open": "open", "high": "high", "low": "low", "close": "close"})
            df["date"] = pd.to_datetime(df["date"])
            for c in ["open", "high", "low", "close"]:
                df[c] = pd.to_numeric(df[c], errors="coerce")
            df = df.dropna(subset=["date", "open", "high", "low", "close"]).sort_values("date").reset_index(drop=True)
            df = df[df["date"].dt.date >= start].reset_index(drop=True)
            if len(df) < 60:
                raise ValueError(f"日线有效数据不足: {sym}, rows={len(df)}")
            return df
        except Exception as e:
            last_err = e
    raise last_err
def _m30_cache_path() -> Path:

    try:
        return Path(__file__).with_name("zz500_m30_cache.csv")
    except Exception:
        return Path("zz500_m30_cache.csv")

def _merge_m30_cache(df_new: pd.DataFrame, keep_days: int = 60) -> pd.DataFrame:
    """把当日/最近一日30min 数据落盘，跨日累积，解决新浪分时接口只能拿最近交易日的问题。"""
    p = _m30_cache_path()
    if p.exists():
        try:
            df_old = pd.read_csv(p)
            if "dt" in df_old.columns:
                df_old["dt"] = pd.to_datetime(df_old["dt"])
    ...
    df = df.dropna(subset=["dt", "open", "high", "low", "close"]).drop_duplicates(subset=["dt"]).sort_values("dt").reset_index(drop=True)

    # 只保留最近 keep_days 天
    cut = (df["dt"].max().normalize() - pd.Timedelta(days=keep_days))
...
def fetch_m30(now_cn: datetime) -> pd.DataFrame:
    """
    30min 获取策略：
    1) 优先：index_zh_a_hist_min_em（东财，支持区间，适合做 30m ATR） 
    2) 如果网络导致东财不可用，则用新浪 stock_zh_a_minute(period=30) 拿“最近交易日”并用本地缓存跨日累积 
    """
    start = now_cn - timedelta(days=LOOKBACK_DAYS_M30)

    # --- 1) 东财（区间） ---
    if USE_EASTMONEY_M30:
        try:
            ...
            # 顺手写入缓存，方便以后即便东财挂了也能续上
            _merge_m30_cache(df[["dt","open","high","low","close"]].copy(), keep_days=90)
            return df
        except Exception:
            pass
    ...
    for sym in (SINA_SYMBOL_PRIMARY, SINA_SYMBOL_FALLBACK):
        try:
            df = ak.stock_zh_a_minute(symbol=sym, period="30", adjust="")
            if df is None or len(df) == 0:
                raise ValueError(f"stock_zh_a_minute 返回空: {sym}")

            # 输出字段：day/open/high/low/close/volume
            ...
            # 跨日累积
            df_all = _merge_m30_cache(df[["dt","open","high","low","close"]].copy(), keep_days=90)

            # 重新算 atr30（需要多日数据的话靠缓存累积）
            df_all["d"] = df_all["dt"].dt.normalize()
            df_all["atr30"] = wilder_atr_core(df_all["high"], df_all["low"], df_all["close"], M30_ATR_N)
            return df_all
        except Exception as e:
            last_err = e
...
def fetch_spot() -> dict | None:
    """实时行情（新浪）：stock_zh_index_spot_sina 单次返回全部指数，再过滤出 CSI500 """
    df = ak.stock_zh_index_spot_sina()

    row = None
    if "代码" in df.columns:
        hit = df[df["代码"].astype(str).isin([SINA_SYMBOL_PRIMARY, SINA_SYMBOL_FALLBACK])]
        if len(hit) > 0:
            row = hit.iloc[0]
...
def main():
    ...
    daily_sig_cache = None
    daily_cache_day = None
    ...
    while True:
        try:
            ...
            # 拉取数据（日线用缓存：一天刷新一次；失败会重试 2 次）
            now_day = now_cn.date()
            if (daily_sig_cache is None) or (daily_cache_day != now_day):
                last_err = None
                for _ in range(2):
                    try:
                        daily = fetch_daily(now_cn)
                        daily_sig_cache = compute_signals_and_atr(daily)
                        daily_cache_day = now_day
                        last_err = None
                        break
                    except Exception as e:
                        last_err = e
                        time.sleep(1.0)
                if last_err is not None and daily_sig_cache is None:
                    raise last_err

            daily_sig = daily_sig_cache
