...
M30_FILE = Path(r"C:\Users\24106\Desktop\沪深300.xlsx")
M30_SHEET = 0

# 日线：AkShare 拉取沪深300
DAILY_SYMBOL = "sh000300"   # 沪深300

# 对齐区间（建议至少让日线起点 >= 30min 覆盖起点，否则很多信号会被 skip）
DATE_START = "2023-01-30"   # 例如 "2025-07-21"
...
# ==========================================================
# 1) 日线：AkShare 拉取沪深300（日线）
# ==========================================================
def fetch_daily_hs300(symbol: str) -> pd.DataFrame:
    ...
    if DATE_START is None and DATE_END is None:
        df = ak.stock_zh_index_daily_em(symbol=symbol).copy()
    else:
        ...
        df = ak.stock_zh_index_daily_em(symbol=symbol, start_date=sd, end_date=ed).copy()
...
def acceptance_exit_price(day_bars: pd.DataFrame, start_bar_index: int, W: int) -> float | None:
    ...
    if end >= len(day_bars):
        return None
    return float(day_bars.iloc[end]["close"])  # 概率失败：用窗口最后一根 close
...
if __name__ == "__main__":
    # 1) 日线：AkShare
    df_daily = fetch_daily_hs300(DAILY_SYMBOL)
    print(f"[D] {DAILY_SYMBOL} rows={len(df_daily)}, range={df_daily['date'].min().date()} -> {df_daily['date'].max().date()}")
