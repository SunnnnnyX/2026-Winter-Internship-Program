...
DAILY_SYMBOL = f"csi{INDEX_CODE}"   # 优先：stock_zh_index_daily_em 使用 csi+代码
...
def fetch_daily(now_cn: datetime) -> pd.DataFrame:
    """
    日线获取（健壮版）：
    1) 优先 stock_zh_index_daily_em(symbol="csi000905") —— 但某些 AkShare 版本/东财接口会返回 data=None
    2) 失败则用 index_zh_a_hist(symbol="000905", period="daily") —— 更稳定
    3) 仍失败则抛错，由上层重试/降级

    你遇到的报错：TypeError("'NoneType' object is not subscriptable")
    通常是东财返回 {"data": None, ...}，这段会自动切换到备用接口。
    """
    start = now_cn - timedelta(days=LOOKBACK_DAYS_DAILY)

    start_ymd = _fmt_ymd(start)
    end_ymd = _fmt_ymd(now_cn)

    last_err = None

    # --- 1) 优先：stock_zh_index_daily_em（支持 csi+代码，要求 AkShare 较新版本） ---
    try:
        df = ak.stock_zh_index_daily_em(symbol=DAILY_SYMBOL, start_date=start_ymd, end_date=end_ymd)
        ...
        if len(df) > 0:
            return df
        raise ValueError("stock_zh_index_daily_em 清洗后为空")
    except Exception as e:
        last_err = e

    # --- 2) 备用：index_zh_a_hist（更通用更稳，symbol 不带市场前缀） ---
    try:
        df = ak.index_zh_a_hist(symbol=INDEX_CODE, period="daily", start_date=start_ymd, end_date=end_ymd)
        ...
        if "date" not in df.columns:
            raise ValueError(f"index_zh_a_hist 列名不包含日期列，实际列={list(df.columns)}")
        ...
        # 抛出最有信息量的错误
        raise RuntimeError(
            "日线接口都失败了。建议：先升级 akshare（pip install -U akshare），"
            "或检查网络/东财是否临时限制。"
        ) from (e2 if e2 is not None else last_err)
