...
# 3) 日线信号（做空：三根K递减）
# ==========================================================
def build_signal_daily_short(df: pd.DataFrame) -> pd.Series:
...
def acceptance_prob_short(day_bars: pd.DataFrame, start_bar_index: int, level: float, W: int, tol: float) -> float | None:
    end = start_bar_index + W - 1
    if end >= len(day_bars):
        return None
    w = day_bars.iloc[start_bar_index:end+1]
    # 做空：要求 close <= level + tol
...
# ==========================================================
# 5) 回测：只做空（思路不变，方向全部镜像）
# ==========================================================
def backtest_short(df_daily: pd.DataFrame, m30_map: dict | None) -> dict:
    df = df_daily.copy()
    df["atr"] = wilder_atr_df(df, ATR_N)
    df["signal"] = build_signal_daily_short(df)

    if USE_TREND_FILTER:
        df["ma_fast"] = df["close"].rolling(MA_FAST).mean()
        df["ma_slow"] = df["close"].rolling(MA_SLOW).mean()
        # 做空趋势过滤（如果你要开）：例如 close < ma_slow 且 ma_fast < ma_slow
    ...
    def close_pos(i: int, exit_px: float, reason: str):
        nonlocal cash, pos, entry_px, entry_i, entry_ts, stop, R, lowest_close, min_low_since_entry
        # 做空：盈利 = (entry - exit) * multiplier
        ...
        trades.append({
            ...
            "exit": float(exit_px),
            "pnl": float(pnl),
            "R_mult": float((entry_px - exit_px) / R) if R > 0 else np.nan,
            "reason": reason
        })
        pos = 0
        entry_px = np.nan
    ...
    for i in range(n):
        ...
        if pos == -1:
            ...
            # 还持仓 -> 日线 ATR 吊灯 / 保本 / 超时
            if pos == -1:
                lowest_close = min(lowest_close, float(df.loc[i, "close"]))
                min_low_since_entry = min(min_low_since_entry, float(df.loc[i, "low"]))

                atr_i = float(df.loc[i, "atr"])
                if np.isfinite(atr_i) and atr_i > 0:
                    # 做空吊灯：stop 随着有利方向下移（取 min）
                    trail = lowest_close + K_ATR * atr_i
                    stop = min(stop, trail)

                # 保本：价格向下走到 entry - BE_R*R，则 stop 下移到 entry
                if USE_BREAK_EVEN and np.isfinite(R) and R > 0:
                    if float(df.loc[i, "low"]) <= entry_px - BE_R * R:
                        stop = min(stop, entry_px)

                # 日线止损触发：high >= stop
                if float(df.loc[i, "high"]) >= stop:
                    close_pos(i, stop + SLIPPAGE_PTS, reason="daily_stop")

                # 超时：若 TIMEOUT_T 天内没到达目标，则按当日收盘出
            ...
            if first_idx >= 0 and np.isfinite(df.loc[i, "atr"]):
                ...
                if USE_M30_EXECUTION and (m30_map is not None):
                    ...
                    if day_bars is None or len(day_bars) < (M30_NEED_BARS + 1):
                        ...
                        third_idx = find_threebar_fall_idx(day_bars)
                        if third_idx is not None:
                            ep = pick_entry_price_time(day_bars, third_idx)
                            if ep is not None:
                                entry0, ts, entry_bar_index = ep
                                entry_price = float(entry0) - SLIPPAGE_PTS  # 做空更差的成交：略低卖出（保守）
                                R0 = daily_stop - entry_price  # 做空风险距离必须 >0
                                ...
                                if R0 > 0:
                                    ...
                                    # 入场瞬间：m30_stop = entry + k*ATR30；stop = min(日线止损, m30_stop)
                                    if USE_M30_ATR_STOP:
                                    ...
                                    if did_enter and USE_ACCEPTANCE:
                                        ...
                                        acc = acceptance_prob_short(day_bars, entry_bar_index, level, ACCEPT_W, tol)
                ...
                # fallback：没分钟数据就按日线开盘做空（可选）
                if (not did_enter) and (not USE_M30_EXECUTION or M30_MISSING_POLICY == "fallback"):
                    entry_price = float(df.loc[i, "open"]) - SLIPPAGE_PTS  # 做空更差成交
                    R0 = daily_stop - entry_price
                    if R0 > 0:
                        pos = -1
                        entry_px = entry_price
                        entry_i = i
        ...
        # 3) 权益（日线收盘）
        if pos == -1:
            equity[i] = cash + (entry_px - float(df.loc[i, "close"])) * MULTIPLIER
        else:
            equity[i] = cash
...
if __name__ == "__main__":
    ...
    out = backtest_short(df_daily, m30_map=m30_map)
